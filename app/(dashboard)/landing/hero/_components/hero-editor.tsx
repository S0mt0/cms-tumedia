...
export function HeroEditor({
  ...
  return (
    <section
      ...
      data-automated-test-id="landing-hero-editor"
    >
      ...
      >
        <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_minmax(18rem,.62fr)]">
          <div className="space-y-5">
            ...
            <CtaButtonsEditor
              onChange={(ctas) => setDraft((current) => ({ ...current, ctas }))}
              readOnly={readOnly}
              value={draft.ctas}
            />
          </div>
          ...
          <aside className="space-y-5 2xl:border-l 2xl:border-[#d7e1dc] 2xl:pl-6">
            <section className="overflow-hidden rounded-md border border-[#c5d4cd] bg-[#f8fbf9]">
              <div className="flex items-center justify-between gap-3 border-b border-[#d7e1dc] px-4 py-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-[#61746d]">
                    {draft.backgroundMedia.type === "image"
                      ? "Image"
                      : "Looping MP4 video"}
                  </p>
                </div>
              ...
              <MediaPreview
                ...
                media={draft.backgroundMedia}
              />
              <div className="px-4 py-3 text-xs leading-5 text-[#61746d]">
                {draft.backgroundMedia.type === "image"
                  ? draft.backgroundMedia.alt
                  : draft.backgroundMedia.alt ||
            ...
            <div>
              ...
              <Input
                ...
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    scrollLabel: event.target.value,
                  }))
                }
                readOnly={readOnly}
                value={draft.scrollLabel}
              />
            </div>
            <p className="rounded-md border border-[#d4e0da] bg-[#eff7f3] px-4 py-3 text-sm leading-6 text-[#52736a]">
              Changes are saved only when this form differs from its latest
              published values.
            </p>
      ...
      <MediaUploadDialog
        ...
        onSelect={selectBackgroundMedia}
        open={mediaDialogOpen}
        value={draft.backgroundMedia}
      />
    </section>
  );
}
