...
export default async function PerspectivePage() { const initial = await getSection(); return <CmsEditorPageShell eyebrow="About page" title="Our perspective" description="The brand and creator needs that shape the TU Media approach." breadcrumbs={[{ label: "Overview", href: "/" }, { label: "About", href: "/about" }, { label: "Our perspective" }]} previewHref={`${process.env.FRONTEND_BASE_URL ?? "http://localhost:3001"}/about`}><AboutEditor section="perspective" initial={initial} title="Perspective content" description="Edit the introduction and ordered brand and creator need lists." mediaPreviewBaseUrl={process.env.FRONTEND_BASE_URL ?? "http://localhost:3001"} /></CmsEditorPageShell>; }
