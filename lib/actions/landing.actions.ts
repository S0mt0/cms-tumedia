...
import { getLandingSectionDefinition } from "@/lib/constants/landing-sections";
...
export async function updateLandingSection(
  ...
  const sectionData = landingSectionSchemas[parsed.data.section].safeParse(
    parsed.data.data
  );
  ...
  if (!sectionData.success)
    return {
      success: false,
      message: "Please correct the section fields.",
      fieldErrors: sectionData.error.flatten().fieldErrors,
    };

  await landingRepository.updateSection(
    parsed.data.section,
